...
if is_dev and is_test:
    raise RuntimeError("DEV and TESTING must not both be True.")

# 2. Read DB settings from environment
if is_dev:
    DATABASE_URL = os.getenv("TEST_DATABASE_URL")
    missing_vars = [] if DATABASE_URL else ["TEST_DATABASE_URL"]
elif is_test:
    DATABASE_URL = os.getenv("PYTEST_DATABASE_URL")
    missing_vars = [] if DATABASE_URL else ["PYTEST_DATABASE_URL"]
else:
